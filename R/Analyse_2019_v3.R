...
#------------------------------------------------------------------------------------------------------
# Figure: Deceived by p-values (CI)
#------------------------------------------------------------------------------------------------------
...
mod12 <- lm(Richtig ~ log(experience + 1) * p_value_shown + observer_sex, data = dattot)
summary(mod12)

#------------------------------------------------------------------------------------------------------
# Confidence
#------------------------------------------------------------------------------------------------------
mod <- lm(confidence ~ log(experience + 1) * p_value_shown, data = groupdat19)
summary(mod)
...
#------------------------------------------------------------------------------------------------------
# Figure: Deceived by p-values (CI), with Confidence instead of experience as predictor
#------------------------------------------------------------------------------------------------------
#Model 2019
dat19CI <- subset(dat19, CI == 1)

mod <- glmer(say_correct ~ confidence * p_value_shown + (1|ID_person), 
               family = binomial, data = dat19CI)
summary(mod)

nsim <- 100
newdat <- expand.grid(confidence = 1:10, pvalue = c(0, 1))
simres <- sim(mod10, nsim)@fixef
tmpres <- array(NA, dim = c(nrow(newdat), nsim))
for(x in 1:nsim) {
  tmpres[, x] <- plogis(simres[x, 1] + 
                          simres[x, 2] * newdat$confidence + 
                          simres[x, 3] * newdat$pvalue + 
                          simres[x, 4] * newdat$confidence * newdat$pvalue)
}
newdat$mean <- apply(tmpres, 1, mean)
newdat$low <- apply(tmpres, 1, quantile, probs = 0.025)
newdat$up <- apply(tmpres, 1, quantile, probs = 0.975)
newdat$pvaluel <- "no p-values presented"
newdat$pvaluel[newdat$pvalue == 1] <- "p-values presented"
ggplot(newdat) +  ylim(0, 1) + scale_x_continuous(breaks = 1:10) +
  aes(confidence, mean, fill = pvaluel) +
  geom_ribbon(aes(ymin = low, ymax = up), alpha = 0.3) +
  geom_line(aes(colour = pvaluel), lwd = 1.5) +
  labs(x = "Confidence", y = "Proportion of correct answers",
       title = "2019 glmer with interaction confidence X p-value; CI only") +
  theme(legend.title=element_blank()) +
  theme(legend.position="bottom")
